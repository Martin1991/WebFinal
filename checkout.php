...
$sql="SELECT tour_order.order_id, tour.tour_name, tour.tour_description, tour.tuser_id, tour.location, tour.cover_url, tour.tour_id, tour_details.price, guider.profile_image, tour_details.ending_time-tour_details.starting_time as duration, register.user_name 
      FROM tour_order
		LEFT OUTER JOIN tour_details
			ON tour_order.otour_id = tour_details.ttour_id
		LEFT OUTER JOIN tour
			ON tour.tour_id = tour_details.ttour_id
		LEFT OUTER JOIN guider
			ON guider.guider_id = tour.tuser_id
		LEFT OUTER JOIN register
			ON register.user_id = guider.guider_id
	  WHERE tour_order.customer_id='$userId' AND tour_order.checkout='0'";
...
<html xmlns='http://www.w3.org/1999/xhtml'>
	...
	<body class='main-container'>
		...
		<div class='result_contain container'>
			<p class='head_title'>Check Out</p>
...
while($row=mysqli_fetch_array($result))
{
					...
					<div class='item_content col-md-9'>
						<div class='content_up'>
							<div class='item_head_part'>
								...
								<div class='guidercard_info'>
									...
									<input type='hidden' id='userId' name='userId' value='".$row['tuser_id']."'/>
									<noscript>
						...
						<div class='content_down '>
							<div class='book_button'>
								<form id='delete_item' action='delete_order.php' method='POST'>
									<input type='hidden' id='orderId' name='orderId' value='".$row['order_id']."'/>
									<input type='submit' id='book_button' value='Delete' class='book_button btn btn-success'/>
								</form>
	...
	echo "
			...
			<div class='order_total'>Order total: $".$order_total."</div>";
			//date_default_timezone_set('America/Chicago');
			$date = 1000;//date('m/d/Y h:i:s a', time());
			echo "
			<div class='checkout_div'>
				<form id='checkout_form' action='deal_checkout.php' method='POST'>
					<input type='hidden' id='userId' name='userId' value='".$userId."'/>
					<input type='hidden' id='date_time' name='date_time' value='".$date."'>
					<input type='submit' id='checkout_button' value='Place your order' class='checkout btn btn-success'/>
				</form>
			</div>
		</div>
		<!--login-->
