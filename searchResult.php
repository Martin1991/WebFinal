...
//	 WHERE tour.tour_id=tour_details.ttour_id AND tour.tuser_id=register.user_id";
...
$sql="SELECT tour.tour_name, tour.tour_description, tour.location, tour.cover_url, tour.tour_id, tour_details.price, guider.profile_image, register.user_id, tour_details.ending_time-tour_details.starting_time as duration, register.user_name 
     FROM tour_details
		...
		WHERE tour.tour_id=tour_details.ttour_id";
	
if(!empty($Category))
{
	$sql.= " AND tour.category='$Category'";
}
if(!empty($Location))
{
	$sql.= " AND tour.location='$Location'";
}
if(!empty($Price))
{
...
if(!empty($StartTime))
{
	$dob_1=DateTime::createFromFormat('m/d/Y', $StartTime);
	$StartTime=$dob_1->format('Y-m-d');
	$sql.=" AND DATEDIFF(tour_details.starting_time,'$StartTime')>=0";
}
if(!empty($EndTime))
{
	$dob_2=DateTime::createFromFormat('m/d/Y', $EndTime);
	$EndTime=$dob_2->format('Y-m-d');
	$sql.=" AND DATEDIFF('$EndTime',tour_details.ending_time)>=0";
}
$sql.=";";

	//$sql="SELECT tour.tour_name, tour.tour_description, tour.location, tour.cover_url, tour_details.price, tour_details.ending_time-tour_details.starting_time as duration, register.user_name FROM tour, tour_details, register
	//WHERE tour.tour_id=tour_details.ttour_id AND tour.tuser_id=register.user_id AND tour.category='$Category' AND tour.location='$Location' AND tour_details.price='$Price' AND DATEDIFF(tour_details.starting_time,'$StartTime')>=0 AND DATEDIFF('$EndTime',tour_details.ending_time)>=0;";
...
while($row=mysqli_fetch_array($result))
{
	...
	<div class='item_result container'>
					<div class='tourPic_div col-md-3'>
						<img class='tour_pic' src='".$row['cover_url']."' value='".$row['location']."' alt='travel in ".$row['location']."' title='travel in ".$row['location']."' action='tourDetail.php' method='get'/>
					</div>
					<div class='item_content col-md-9'>
						<div class='content_up'>
							<div class='item_head_part'>
								...
								<div class='guidercard_info'>
									...
									<img class='guider_pic' src='".$row['profile_image']."' value='".$row['user_name']."' alt='".$row['user_name']."' title='".$row['user_name']."' action='guiderDetail.php' method='get'/>
						...
						<div class='content_down '>
							<div class='book_button'>
								<form id='search_tourDetail' name='search_tourDetail' action='detail.php' method='POST'>
									<input type='hidden' class='tourId' name='tourId' value='".$row['tour_id']."'/>
									<input type='submit' id='submit_searchDetail' value='Book tour' class='book_button btn btn-success'/>
								</form>
							</div>
